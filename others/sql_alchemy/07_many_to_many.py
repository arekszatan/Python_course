...
class Article(Base):
    ...
    # W relacji "many to many", secondary wskazujhe na tabel≥ę łączącą
    # (czasami nazywaną tabelą stowarzysze ń lub tabelą pośrednią),
    # która przechowuje klucze obce obu powiązanych tabel.
    # Tabela łącząca zawiera co najmniej dwa klucze obce, które odnoszą się
    # doi głównych kluczy każdej z powiązanych tabel.
...
def get_all_articles():
    with Session(engine) as session:
        # Używamy joinload, możemyt załadować powiązane obiekty w ramach tego
...
add_tags_if_not_exists(["Python", "SQL", "Advanced"])
...
for article in articles:
    ...
    print(f"Artukuł: {article.title}, Tagi: {tag_names}")